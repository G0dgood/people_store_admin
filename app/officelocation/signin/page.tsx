...
import { useCustomerLoginMutation } from "@/lib/redux/services/customerAuthApi";
...
function CustomerSigninPageContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const { storeContext, updateStoreContext } = useUserInfo();
  const auth = useAuth();
  const [login, { isLoading, isError, error }] = useCustomerLoginMutation();
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <form className="mt-10 space-y-6" onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between">
            ...
            />
            <Link
              href={`/customer/signup${getCustomParams()}`}

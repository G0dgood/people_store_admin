  ...
  return (
    <section className="w-full border border-gray-200 overflow-hidden bg-white mt-8 mb-12">
      <div className="p-6 border-b border-gray-200 flex items-center justify-between">
        <h3 className="text-xl font-bold text-gray-900 tracking-tight uppercase tracking-widest">Recently viewed</h3>
        <Link href="/products" className="text-[10px] font-black uppercase tracking-[0.2em] text-brand-gold hover:tracking-[0.3em] transition-all">
          Explore Boutique Selection
        </Link>
      </div>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 xl:grid-cols-5 border-t border-gray-100">
        {recentlyViewedItems.slice(0, 5).map((item, idx) => (
          <div key={item.id} className="border-r border-b border-gray-100 last:border-r-0 lg:[&:nth-child(4)]:border-r-0 xl:[&:nth-child(5)]:border-r-0">
            <ProductGridItem
              product={{
                ...item,
                rating: 0,
                orders: 0,
                shipping: "History",
                description: "Previously viewed item from your collection.",
                stock: item.stock || 0,

...
export const AdminNotificationDropdown: React.FC = () => {
  ...
  return (
    <div className="absolute top-full right-[-80px] pt-4 z-50 cursor-default">
      <DropdownMenu width={360} className="shadow-2xl border-gray-100 p-0">
        <div className="px-5 py-4 border-b border-gray-50 flex justify-between items-center bg-gray-50/50">
          ...
          <button className="text-[10px] font-black text-brand-blue uppercase hover:underline">Mark all as read</button>
        </div>
        ...
        <div className="flex flex-col max-h-[400px] overflow-y-auto custom-scrollbar">
          {notifications.map((item) => (
            <div 
              key={item.id} 
              className={`
                px-5 py-4 hover:bg-gray-50 flex gap-4 cursor-pointer transition-all border-b border-gray-50/50 last:border-0 relative group
                ${!item.isRead ? "bg-blue-50/20" : ""}
              `}
            >
              <div className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 shadow-sm ${typeStyles[item.type].bg}`}>
              ...
              {!item.isRead && (
                <div className={`w-2 h-2 rounded-full absolute top-1/2 -translate-y-1/2 right-5 shadow-sm ${typeStyles[item.type].dot}`}></div>
              )}
            </div>
          ))}
        </div>

          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            {filteredItems.map((item) => (
              <div
                ...
                className="group relative aspect-square bg-white border border-gray-100 rounded-xl overflow-hidden cursor-pointer hover:border-brand-gold transition-all   hover:shadow-lg"
              >
                ...
                <div className="absolute bottom-2 left-2 right-2 flex justify-between items-center opacity-0 group-hover:opacity-100 transition-opacity translate-y-2 group-hover:translate-y-0 duration-300">
                  <span className="text-[8px] font-bold bg-white/90 backdrop-blur-sm text-gray-900 px-1.5 py-0.5 rounded   uppercase tracking-tighter">
                    {item.type}
                  </span>
                </div>
              </div>

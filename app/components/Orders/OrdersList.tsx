...
export const OrdersList: React.FC = () => {
 ...
 const [detailModalMode, setDetailModalMode] = useState<"view" | "review">("view");
 ...
 const handleViewDetails = (order: any, mode: "view" | "review" = "view") => {
        ...
        <div className="flex flex-wrap gap-3">
         {order.items?.map((item: any, idx: number) => (
          <div key={idx} className="flex flex-col gap-2">
           <div className="w-20 h-20 md:w-32 md:h-32 flex-shrink-0 border border-gray-200 flex items-center justify-center p-2 bg-white overflow-hidden relative group/item">
            {item.product?.productImage ? (
             <div className="relative w-full h-full transition-transform duration-500 group-hover/item:scale-110">
              <Image src={item.product.productImage} alt={item.product.name} fill className="object-contain" sizes="100px" />
             </div>
            ) : (
             <Icon name="perfume_empty" size="md" className="text-gray-200" />
            )}
           </div>
           {order.status === "Delivered" && (
            <button
             ...
             className="text-[8px] font-black uppercase tracking-widest text-gray-400 hover:text-brand-gold transition-colors text-center"
            >
             Review
            </button>
           )}
          ...
          <Button
           ...
           onClick={() => {
            if (order.items.length === 1) {
             ...
             handleViewDetails(order, "review");
            }
           }}
           variant="outline"
          >

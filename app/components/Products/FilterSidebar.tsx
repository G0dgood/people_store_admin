       ...
       return (
        <React.Fragment key={cat?._id || cat?.name}>
         ...
         <li
          ...
          className={`cursor-pointer transition-all duration-300 hover:tracking-[0.15em] ${isSelected ? "text-brand-gold" : "text-gray-500 hover:text-brand-gold"}`}
         >
          {cat.name}
         </li>
         {isSelected && cat.subCategories && cat.subCategories.length > 0 && (() => {
          ...
          return (
           <ul className="pl-4 flex flex-col gap-2 mt-1.5 mb-2.5 normal-case tracking-normal font-medium text-xs text-gray-500">
            {uniqueSubs.map((sub: string) => {
             ...
             return (
              <li
               ...
               className={`cursor-pointer hover:text-brand-gold transition-colors flex items-center gap-1.5 ${isSubSelected ? "text-brand-gold font-bold" : ""}`}
              >
               <span className={`w-1 h-1 rounded-full bg-current opacity-60 ${isSubSelected ? "opacity-100" : "invisible"}`} />
               {sub}
              </li>
             );
            })}

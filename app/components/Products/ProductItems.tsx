...
export const ProductListItem: React.FC<{ product: ProductProps }> = ({ product }) => {
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-3 md:p-5 flex gap-3 md:gap-6 hover:shadow-md transition-shadow cursor-pointer relative group">
      {/* Product Image */}
      <div className="w-24 h-24 md:w-48 md:h-48 flex-shrink-0 border border-gray-100 rounded flex items-center justify-center p-2 md:p-4 bg-white">
         <div className="relative w-full h-full">
           <Image src={product.image} alt={product.title} fill className="object-contain" />
         </div>
      </div>
      ...
      <div className="flex-1 flex flex-col gap-1 md:gap-3 pr-8 md:pr-0">
         <div className="flex items-start justify-between">
            ...
            {/* Heart Icon (Desktop) */}
            <button className="hidden md:flex w-9 h-9 border border-gray-200 rounded-md items-center justify-center hover:text-red-500 hover:border-red-500 transition-colors flex-shrink-0">
               <Icon name="favorite" size="sm" />
            </button>
         </div>
         ...
         <Link href="/products/detail" className="hidden md:block text-brand-blue font-bold text-sm w-fit hover:underline pt-2 cursor-pointer">
            View details
         </Link>
      </div>

      {/* Heart Icon (Mobile - Floating like the design) */}
      <button className="md:hidden absolute top-3 right-3 text-gray-400 hover:text-red-500 transition-colors">
         <Icon name="favorite" size="sm" />
      </button>
    </div>
  );

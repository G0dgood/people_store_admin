...
import { UsersMoreActionsDrawer } from "@/app/components/Admin/UsersMoreActionsDrawer";
import { StaffDetailDrawer } from "@/app/components/Admin/StaffDetailDrawer";
...
import { useGetUsersQuery, useDeleteStaffMutation, useBulkDeleteStaffMutation, useBulkLockStaffMutation } from "@/lib/redux/services/authApi";
...
export default function UsersManagement() {
 ...
 const [bulkDelete, { isLoading: isBulkDeleting }] = useBulkDeleteStaffMutation();
 const [bulkLock, { isLoading: isBulkLocking }] = useBulkLockStaffMutation();

 const handleBulkDelete = async () => {
  try {
   const result = await bulkDelete(selectedIds).unwrap();
   toast.success("Bulk Action Successful", {
    description: result.message || "Selected staff members have been removed."
   });
   setSelectedIds([]);
  } catch (err: any) {
   toast.error("Bulk Action Failed", {
    description: err.data?.message || "Failed to remove selected staff."
   });
  }
 };
 ...
 const handleBulkLock = async (status: "Active" | "Suspended") => {
  try {
   const result = await bulkLock({ userIds: selectedIds, status }).unwrap();
   toast.success("Status Updated", {
    description: result.message || `Selected staff members are now ${status}.`
   });
   setSelectedIds([]);
  } catch (err: any) {
   toast.error("Status Update Failed", {
    description: err.data?.message || "Failed to update staff status."
   });
  }
 };
 ...
 return (
  <div className="flex flex-col gap-6 max-w-[1600px] mx-auto pb-12">
         ...
         <tr key={user._id} className="group">
          ...
          <td>
           <div className="flex items-center gap-2">
            <span className={`w-1.5 h-1.5 rounded-full ${user.status === 'Inactive' || user.status === 'Suspended' ? 'bg-gray-300' : 'bg-emerald-500 animate-pulse'}`} />
            <span className={`px-2.5 py-1 rounded-[6px] text-[10px] font-black ${user.status === 'Inactive' || user.status === 'Suspended' ? statusStyles.Inactive : statusStyles.Active}`}>
             {user.status || 'Active'}
            </span>
           </div>
          </td>
          <td className="text-right pr-6">
           <div className="flex justify-end gap-3 transition-all duration-300">
            ...
            {canAccess("users", "edit") && (
             <Tooltip text="Edit Staff Member">
              <Button shape="rounded-sm" variant="outline"
   ...
   <BulkActionsDrawer
    ...
    isLoading={isBulkDeleting || isBulkLocking}
    actions={[
     {
      id: "export",
     ...
     },
     {
      ...
      icon: "block",
      folder: "icon",
      onClick: () => handleBulkLock("Suspended"),
     },
     {
      id: "activate",
